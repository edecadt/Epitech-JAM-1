...
class Map:
    ...
    def update(self, x_to_move, player, color_map):
        if player.y >= 10:
            ...
            if x_to_move < 0 and color_map.image.get_at((-color_map.map_x + player.x + 40 + player.speed, player.y + 99))[1] > 100:
                return
            # collision between player and objects on its left (width -> 100, height -> 100)
            if x_to_move > 0 and color_map.image.get_at((-color_map.map_x + player.x - player.speed, player.y + 99))[1] > 100:
                return
